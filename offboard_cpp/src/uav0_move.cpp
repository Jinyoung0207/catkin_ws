...
private:
    ...
    ros::Subscriber ref_path_sub; //경로 생성 노드에서 보내는 경로를 구독해서 set_point_pub으로 발행하는거임
...
public:
    ...
    void run() 
    {
        ...
        while (ros::ok()) 
        {
            ...
            pose0.pose.position.x = ref_path_msg.poses[0].pose.position.x; //ref_path의 첫번째 값을 저장
            pose0.pose.position.y = ref_path_msg.poses[0].pose.position.y; //ref_path에는 refpoint값이 들어가있음.
            pose0.pose.position.z = ref_path_msg.poses[0].pose.position.z;
            
            
            set_point_pub.publish(pose0);//refpoint 첫번째 값을 mavros/setpoint_position/local에 발행 드론이 따라가야할 첫번째 셋포인트를 발행해주게되면 드론이 따라감
                                         //NUC에서 진행될 절차
            ros::spinOnce();
            rate.sleep();
        }
