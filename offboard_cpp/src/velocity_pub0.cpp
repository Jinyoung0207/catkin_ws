...
public:
    ...
    {   //각각 0,1,2의 위치 구독함으로써 거리 계산 가능
        pose_sub = nh_.subscribe<geometry_msgs::PoseStamped>("uav0/mavros/local_position/pose", 10, &Velocity::poseCallback, this);
        ref_path_sub = nh_.subscribe<nav_msgs::Path>("uav0/ref_path", 10, &Velocity::refpathCallback, this);
        state_sub = nh_.subscribe<mavros_msgs::State>("uav0/mavros/state", 10, &Velocity::stateCallback, this);
    ...
    void run()
    {
        ...
        while (ros::ok()) 
        {
            ...
            x = ref_path_msg.poses[0].pose.position.x-current_pose.pose.position.x;
...
{
    ros::init(argc, argv, "velocity_pub0");
