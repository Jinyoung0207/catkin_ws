...
public:
    ...
    {   //각각 0,1,2의 위치 구독함으로써 거리 계산 가능
        pose_sub = nh_.subscribe<geometry_msgs::PoseStamped>("uav1/mavros/local_position/pose", 10, &Velocity::poseCallback, this);
        ref_path_sub = nh_.subscribe<nav_msgs::Path>("uav1/ref_path", 10, &Velocity::refpathCallback, this);
        state_sub = nh_.subscribe<mavros_msgs::State>("uav1/mavros/state", 10, &Velocity::stateCallback, this);
        cmd_vel_pub = nh_.advertise<geometry_msgs::TwistStamped>("/uav1/mavros/setpoint_velocity/cmd_vel", 10);
...
{
    ros::init(argc, argv, "velocity_pub1");
