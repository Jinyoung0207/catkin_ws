...
class ref_path_make5 {
public:
    ref_path_make5() {
        ros::NodeHandle nh;

        global_path_publisher_ = nh.advertise<nav_msgs::Path>("path5", 10);
        ref_path_publisher_ = nh.advertise<nav_msgs::Path>("/rbt5/ref_trajectory", 10);
        subscription_ = nh.subscribe("/global_path5", 10, &ref_path_make5::pathCallback, this);

        global_timer_ = nh.createTimer(ros::Duration(0.1), &ref_path_make5::globalPathMake, this);
        ref_path_timer_ = nh.createTimer(ros::Duration(0.1), &ref_path_make5::refPathMake, this);
    }
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "ref_path_make5");
    ref_path_make5 make_traj05;
    ros::spin();
    return 0;
}