...
void mpc_ros1_rbt5::setting_reference()
{
    for(int n = 0; n < N; ++n)
    {
        double roll, pitch;
...
void mpc_ros1_rbt5::make_args_p()
{
...
void mpc_ros1_rbt5::rbt1_path_Callback(const nav_msgs::Path::ConstPtr& msg){
    buf.lock();
    rbt1_path_queue.push(*msg);
    buf.unlock();
}
void mpc_ros1_rbt5::rbt1_path_update()
{
...
void mpc_ros1_rbt5::rbt2_path_Callback(const nav_msgs::Path::ConstPtr& msg){
    buf.lock();
    rbt2_path_queue.push(*msg);
    buf.unlock();
}
void mpc_ros1_rbt5::rbt2_path_update()
{
...
void mpc_ros1_rbt5::rbt3_path_Callback(const nav_msgs::Path::ConstPtr& msg){
    buf.lock();
    rbt3_path_queue.push(*msg);
    buf.unlock();
}
void mpc_ros1_rbt5::rbt3_path_update()
{
...
void mpc_ros1_rbt5::rbt4_path_Callback(const nav_msgs::Path::ConstPtr& msg){
    buf.lock();
    rbt4_path_queue.push(*msg);
    buf.unlock();
}
void mpc_ros1_rbt5::rbt4_path_update()
{
    if (!rbt4_path_queue.empty())
    {
        buf.lock();
        rbt4_path_msg = rbt4_path_queue.back();
        rbt4_path_queue.pop();
        rbt4_pre_in = true;
        buf.unlock();
    }
}
void mpc_ros1_rbt5::rbt_odom_Callbck(const nav_msgs::Odometry::ConstPtr& msg)
{
    buf.lock();
...
void mpc_ros1_rbt5::rbt_ref_traj_Callbck(const nav_msgs::Path::ConstPtr& msg)
{
    buf.lock();
...
void mpc_ros1_rbt5::avoid_obstacle() {
    ...
    for (int k = 0; k < pre_path_N  ; ++k) {
        for (int j = 0; j < robot_n; ++j) {
            ...
            if (j == 0) {
                ...
                dist = casadi::SX::sqrt(casadi::SX::pow(X(0, k) - rbt4_path_msg.poses[k].pose.position.x, 2) +
                                        casadi::SX::pow(X(1, k) - rbt4_path_msg.poses[k].pose.position.y, 2));
            }
...
void mpc_ros1_rbt5::state_update()
{
...
void mpc_ros1_rbt5::rbt_ref_traj_update()
{
...
void mpc_ros1_rbt5::rbt_odom_update()
{
...
/*
void mpc_ros1_rbt5::publish_obstacles()
{
...
void mpc_ros1_rbt5::node_param_init()
{
    //int N;
    ...
    nh_.getParam("/mpc_ros1_rbt5_node/N", N);
    nh_.getParam("/mpc_ros1_rbt5_node/dt", dt);
    nh_.getParam("/mpc_ros1_rbt5_node/v_max", v_max);
    nh_.getParam("/mpc_ros1_rbt5_node/v_min", v_min);
    nh_.getParam("/mpc_ros1_rbt5_node/cmd_topic_name", cmd_topic_name);
    nh_.getParam("/mpc_ros1_rbt5_node/pose_topic_name", odom_topic_name);
    nh_.getParam("/mpc_ros1_rbt5_node/pre_topic_name", pre_topic_name);
    nh_.getParam("/mpc_ros1_rbt5_node/ref_topic_name", ref_topic_name);
    nh_.getParam("/mpc_ros1_rbt5_node/frame_id", map_id_name);
...
void mpc_ros1_rbt5::init()
{
    rbt_odom_sub = nh_.subscribe<nav_msgs::Odometry>(odom_topic, 1, &mpc_ros1_rbt5::rbt_odom_Callbck, this);
    rbt_ref_traj_sub = nh_.subscribe<nav_msgs::Path>(ref_topic, 1, &mpc_ros1_rbt5::rbt_ref_traj_Callbck, this);
    // rbt1_path_sub = nh_.subscribe<nav_msgs::Path>("/UGV1/pre_path", 1, &mpc_ros1_rbt5::rbt1_path_Callback, this);
    // rbt2_path_sub = nh_.subscribe<nav_msgs::Path>("/UGV2/pre_path", 1, &mpc_ros1_rbt5::rbt2_path_Callback, this);
    // rbt3_path_sub = nh_.subscribe<nav_msgs::Path>("/UGV3/pre_path", 1, &mpc_ros1_rbt5::rbt3_path_Callback, this);
    // rbt4_path_sub = nh_.subscribe<nav_msgs::Path>("/UGV4/pre_path", 1, &mpc_ros1_rbt5::rbt4_path_Callback, this);
...
void mpc_ros1_rbt5::run()
{
    ...
    while (ros::ok())
    {
        ...
        // rbt3_path_update();
        // rbt4_path_update();
        // if (rbt_odom_in && rbt_ref_traj_in && rbt1_pre_in && rbt2_pre_in && rbt3_pre_in && rbt4_pre_in)
...
{
    ros::init(argc, argv, "mpc_ros1_rbt5");  // ROS 1에서는 ros::init을 사용하여 노드를 초기화
    mpc_ros1_rbt5 mpc_node;
