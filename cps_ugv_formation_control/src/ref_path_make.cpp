...
public:
    ref_path_make() {
        ...
        global_path_publisher_ = nh.advertise<nav_msgs::Path>("path1", 10);
        ref_path_publisher_ = nh.advertise<nav_msgs::Path>("/rbt1/ref_trajectory", 10);
        subscription_ = nh.subscribe("/global_path1", 10, &ref_path_make::pathCallback, this);

        global_timer_ = nh.createTimer(ros::Duration(0.1), &ref_path_make::globalPathMake, this);
        ref_path_timer_ = nh.createTimer(ros::Duration(0.1), &ref_path_make::refPathMake, this);
    }
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "ref_path_make");
    ref_path_make make_traj05;
    ros::spin();
    return 0;
}
