...
void mpc_ros1_rbt2::setting_reference()
{
    for(int n = 0; n < N; ++n)
    {
        double roll, pitch;
...
void mpc_ros1_rbt2::make_args_p()
{
...
void mpc_ros1_rbt2::rbt1_path_Callback(const nav_msgs::Path::ConstPtr& msg){
    buf.lock();
    rbt1_path_queue.push(*msg);
    buf.unlock();
}
void mpc_ros1_rbt2::rbt1_path_update()
{
...
void mpc_ros1_rbt2::rbt_odom_Callbck(const nav_msgs::Odometry::ConstPtr& msg)
{
    buf.lock();
...
void mpc_ros1_rbt2::rbt_ref_traj_Callbck(const nav_msgs::Path::ConstPtr& msg)
{
    buf.lock();
...
void mpc_ros1_rbt2::avoid_obstacle() {

    double rob_diam = 0.40;
    double safe_dis = 0.5;

    lbg = casadi::DM::zeros(n_states * (N + 1) + pre_path_N , 1);
    ubg = casadi::DM::inf(n_states * (N + 1) + pre_path_N , 1);
    ...
    for (int k = 0; k < pre_path_N  ; ++k) {
        ...
        casadi::SX dist = casadi::SX::sqrt(casadi::SX::pow(X(0,k) - rbt1_path_msg.poses[k].pose.position.x, 2) +
                                            casadi::SX::pow(X(1,k) - rbt1_path_msg.poses[k].pose.position.y, 2));
        int idx = n_states * (N + 1) + k;
...
void mpc_ros1_rbt2::state_update()
{
...
void mpc_ros1_rbt2::rbt_ref_traj_update()
{
...
void mpc_ros1_rbt2::rbt_odom_update()
{
...
/*
void mpc_ros1_rbt2::publish_obstacles()
{
...
void mpc_ros1_rbt2::node_param_init()
{
    //int N;
    ...
    nh_.getParam("/mpc_ros1_rbt2_node/N", N);
    nh_.getParam("/mpc_ros1_rbt2_node/dt", dt);
    nh_.getParam("/mpc_ros1_rbt2_node/v_max", v_max);
    nh_.getParam("/mpc_ros1_rbt2_node/v_min", v_min);
    nh_.getParam("/mpc_ros1_rbt2_node/cmd_topic_name", cmd_topic_name);
    nh_.getParam("/mpc_ros1_rbt2_node/pose_topic_name", odom_topic_name);
    nh_.getParam("/mpc_ros1_rbt2_node/pre_topic_name", pre_topic_name);
    nh_.getParam("/mpc_ros1_rbt2_node/ref_topic_name", ref_topic_name);
    nh_.getParam("/mpc_ros1_rbt2_node/frame_id", map_id_name);
...
void mpc_ros1_rbt2::init()
{
    rbt_odom_sub = nh_.subscribe<nav_msgs::Odometry>(odom_topic, 1, &mpc_ros1_rbt2::rbt_odom_Callbck, this);
    rbt_ref_traj_sub = nh_.subscribe<nav_msgs::Path>(ref_topic, 1, &mpc_ros1_rbt2::rbt_ref_traj_Callbck, this);
    // rbt1_path_sub = nh_.subscribe<nav_msgs::Path>("/UGV1/pre_path", 1, &mpc_ros1_rbt2::rbt1_path_Callback, this);
...
void mpc_ros1_rbt2::run()
{
    ...
    while (ros::ok())
    {
        ...
        rbt_ref_traj_update();
        rbt1_path_update();
        // if (rbt_odom_in && rbt_ref_traj_in && rbt1_pre_in)
...
{
    ros::init(argc, argv, "mpc_ros1_rbt2");  // ROS 1에서는 ros::init을 사용하여 노드를 초기화
    mpc_ros1_rbt2 mpc_node;
