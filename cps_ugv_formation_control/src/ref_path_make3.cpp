...
class ref_path_make3 {
public:
    ref_path_make3() {
        ros::NodeHandle nh;

        global_path_publisher_ = nh.advertise<nav_msgs::Path>("path3", 10);
        ref_path_publisher_ = nh.advertise<nav_msgs::Path>("/rbt3/ref_trajectory", 10);
        subscription_ = nh.subscribe("/global_path3", 10, &ref_path_make3::pathCallback, this);

        global_timer_ = nh.createTimer(ros::Duration(0.1), &ref_path_make3::globalPathMake, this);
        ref_path_timer_ = nh.createTimer(ros::Duration(0.1), &ref_path_make3::refPathMake, this);
    }
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "ref_path_make3");
    ref_path_make3 make_traj05;
    ros::spin();
    return 0;
}