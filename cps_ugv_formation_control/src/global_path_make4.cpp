...
class global_path_make4 {
public:
    global_path_make4() {
        ros::NodeHandle nh;
        publisher_ = nh.advertise<nav_msgs::Path>("/global_path4", 10);

        // Generate x values from 0 to 15 with step 0.5
        for (double x = 0.0; x <= 15.0; x += 0.5) {
            x_values.push_back(x);
            y_values.push_back(-1 * std::sin(x));  // y = 0 for all x
        }

        timer_ = nh.createTimer(ros::Duration(0.1), &global_path_make4::publishPath, this);
    }
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "global_path_make4");
    global_path_make4 path_publisher;
    ros::spin();
    return 0;
}