...
class ref_path_make2 {
public:
    ref_path_make2() {
        ros::NodeHandle nh;

        global_path_publisher_ = nh.advertise<nav_msgs::Path>("path2", 10);
        ref_path_publisher_ = nh.advertise<nav_msgs::Path>("/rbt2/ref_trajectory", 10);
        subscription_ = nh.subscribe("/global_path2", 10, &ref_path_make2::pathCallback, this);

        global_timer_ = nh.createTimer(ros::Duration(0.1), &ref_path_make2::globalPathMake, this);
        ref_path_timer_ = nh.createTimer(ros::Duration(0.1), &ref_path_make2::refPathMake, this);
    }
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "ref_path_make2");
    ref_path_make2 make_traj05;
    ros::spin();
    return 0;
}